...
This Decision Tree Shiny app allows users to build a model by selecting variables and adjusting key parameters, including the train/test split ratio, minimum split, maximum depth, and complexity parameter. After the model is built, the app presents performance metrics such as RMSE, MAE, and R-squared, while also clearly visualizing the decision tree structure, the distribution of predicted versus actual values, and feature importance. To enhance usability and interactivity, a “Scenario Prediction” section is included, allowing users to input variable values under the current model and obtain a specific prediction result.

![](pic/full%20view.png)

## Control Panel

The left control panel allows users to adjust key model settings, including variable selection, train/test split ratio, minimum split, maximum depth, and complexity parameter. It also includes parameter definitions to help users better understand the meaning of each setting.

![](pic/control panel.png)
...
![](images/clipboard-3162422269.png)

## Model Visualization

Displays the main visual outputs of the trained decision tree model, including the tree structure, predicted vs actual results on the test dataset, and the top 10 most important variables. This section helps users interpret model behavior and evaluate prediction performance.

![](images/clipboard-2026271510.png)

## Scenario Prediction

Allows users to test a custom scenario by entering values for the variables used in the current decision tree. The app then generates the predicted churn probability and displays the corresponding terminal node and decision path.

![](images/clipboard-738519810.png)
